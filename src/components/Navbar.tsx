    ...
    >
      <div className="section-container flex items-center justify-between h-16">
        ...
        >
          ...
          <span className="text-foreground">KS</span>
          <span className="text-primary"> /&gt;</span>
        </button>
            ...
            <ul className="section-container py-4 space-y-1">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <button
                    onClick={() => handleClick(link.href)}
                    className="w-full text-left px-4 py-3 text-muted-foreground hover:text-primary hover:bg-secondary/50 rounded-md transition-colors"
                  >
                    {link.label}
                  </button>
                </li>
